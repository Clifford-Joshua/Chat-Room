...
const ChatBody = () => {
  ...
  const { activeRoom, groupId } = useSelector((store: RootState) => store.user);

  const fetchMessages = async () => {
    try {
      const res = await axios.get(`${url}/groups/group-Messages/${groupId}`);

      const { groupMessage } = res.data;

      setGetMessage(groupMessage);
    } catch (err: unknown) {
  ...
  return (
    <Wrapper>
      <div className="flex gap-4 flex-col h-[82vh] overflow-scroll px-4 py-4">
        {getMessage.map(({ _id, sendBy, message, senderId }) => {
          const isCurrentUser = sendBy === localStorage.getItem("userId");
          const { username, userImage } = senderId as unknown as {
            username: string;
            userImage: string;
          };
            ...
            return (
              <div key={_id}>
                <div className="flex items-end justify-end gap-2 mb-1">
                  <h2 className="text-[0.7rem] font-bold w-m">{username}</h2>
                  <img
                    src={userImage}
                    alt={username}
